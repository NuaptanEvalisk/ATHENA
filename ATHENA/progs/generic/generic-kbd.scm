...
(tm-define (escape-symbol-insert action)
  (cond ((== action "tree:dx")
        ...
        ((== action "tree:id")
         (insert '(math-up "id")))
        ((== action "tree:const")
         (insert '(math-up "const")))
        ((== action "tree:varinjlim")
         (insert '(wide* "lim" "<wide-varrightarrow>")))
        ((== action "tree:varprojlim")
         (insert '(wide* "lim" "<wide-varleftarrow>")))
        ((== action "tree:bij")
         (insert '(above "<longrightarrow>" "1:1")))
        ((== action "tree:simto")
         (insert '(above "<longrightarrow>" "<sim>")))
        ((== action "tree:lim-n-infty")
         (insert '(concat "lim" (rsub (concat "n" "<rightarrow>" "<infty>")))))
        ((== action "tree:varphi")
         (insert '(concat "<varphi>")))
        ((== action "tree:rel")
         (insert '(concat (space "0.27em") "rel")))
        ((== action "tree:angle-brackets")
         (math-bracket-open "<langle>" "<rangle>" 'default))
        ((== action "tree:norm-brackets")
         (math-bracket-open "<||>" "<||>" 'default))
        ((== action "tree:math-ss")
         (make 'math-ss))
        ((== action "tree:math-bf")
         (make 'math-bf))
        ((== action "tree:math-boldsymbol")
         (make-with "math-font-series" "bold"))
        ((== action "tree:math-frak")
         (make-with "math-font" "Euler"))
        ((== action "tree:math-scr")
         (make-with "math-font" "cal*"))
        ((== action "tree:q3")
         (insert '(concat (space "1em") (space "1em") (space "1em"))))
        ((string-starts? action "tree:math-up:")
         (insert `(math-up ,(substring action 13 (string-length action)))))
...
(kbd-map
  ...
  ("S-left" (kbd-select kbd-left-raw))
  ("S-right" (kbd-select kbd-right-raw))
  ("S-up" (kbd-select kbd-up-raw))
  ("S-down" (kbd-select kbd-down-raw))
  ("S-home" (kbd-select kbd-start-line-raw))
  ("S-end" (kbd-select kbd-end-line-raw))
  ("S-pageup" (kbd-select kbd-page-up-raw))
  ("S-pagedown" (kbd-select kbd-page-down-raw))
...
(kbd-map
  ...
  ("emacs:prefix K" (close-document*))
  ("emacs:prefix C-c" (safely-quit-ATHENA))
  ...
  ("M-S-F2" (new-document*))
  ;;("M-C-F2" (clone-window))
  ("F3" (save-buffer-manual))
  ("S-F3" (choose-file save-buffer-as "Save ATHENA file" "texmacs"))
  ("F4" (preview-buffer))
  ("S-F4" (print-buffer))
...
(kbd-map
  ...
  ("std P" (command-palette-show))
  ("std L" (open-document*))
  ("std p" (preview-buffer))
  ("std q" (safely-quit-ATHENA))
  ("std R" (update-document "all"))
  ("std s" (save-buffer-manual))
  ("std S" (choose-file save-buffer-as "Save ATHENA file" "texmacs"))
...
(kbd-map
  ...
  ("gnome home" (kbd-plain-move go-start))
  ("gnome end" (kbd-plain-move go-end))
...
(kbd-map
  ...
  ("macos up" (kbd-plain-move go-start))
  ("macos down" (kbd-plain-move go-end))
  ("macos S-left" (kbd-select kbd-start-line-raw))
  ("macos S-right" (kbd-select kbd-end-line-raw))
...
(kbd-map
  ...
  ("windows left" (kbd-plain-move traverse-left))
  ("windows right" (kbd-plain-move traverse-right))
  ("windows home" (kbd-plain-move go-start))
  ("windows end" (kbd-plain-move go-end))
