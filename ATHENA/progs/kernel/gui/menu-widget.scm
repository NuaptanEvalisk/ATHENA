...
(define (make-menu-label p style . opt)
  ...
  (let ((tt? (and (nnull? opt) (car opt)))
    ...
    (cond ((ui-text? p)                 ; "text"
          ...
          ((tuple? p 'icon 1)           ; (icon "logical-name")
           (widget-icon (cadr p)))
...
(define (interactive-color-native title cmd proposals)
  (let* ((recent (append (map dialog-color-string proposals)
                         (color-picker-recent-colors)))
         (saved (color-picker-saved-colors))
         (result (native-color-dialog title recent saved)))
    (when (pair? result)
      (color-picker-set-saved-colors (cdr result))
      (color-picker-remember-color (car result))
      (cmd (car result)))))

(tm-define (interactive-color cmd proposals)
  (:interactive #t)
  (interactive-color-native "Choose color" cmd proposals))

(tm-define (interactive-background cmd proposals)
  (:interactive #t)
  (interactive-color-native "Choose background" cmd proposals))
