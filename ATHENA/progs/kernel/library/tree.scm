           ...
           (with j (string-previous s i)
             (and (> j 0)
                  (with k (string-previous s j)
                    (utf8-byte-substring s k j))))))))

(define-public (after-cursor)
  (let* ((t (cursor-tree*))
	 (i (cAr (cursor-path*))))
    (cond ((and (tree-atomic? t) (< i (utf8-byte-length (tree->string t))))
	   (with s (tree->string t)
	     (with j (string-next s i)
	       (utf8-byte-substring s i j))))
	  ((tree-atomic? t) #f)
	  ((== i 0) t)
	  (else #f))))
...
(define-public (cursor-on-border? t)
  (let* ((p (cursor-path))
    ...
    (and (== (cDr p) (tree->path t))
         (or (== i 0)
             (if (tree-atomic? t)
                 (== i (utf8-byte-length (tree->string t)))
                 (== i 1))))))
