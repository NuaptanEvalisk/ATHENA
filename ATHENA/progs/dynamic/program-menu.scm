...
(tm-menu (focus-insert-icons t)
  (:require (prog-field-context? t))
  ((balloon (icon "tm_insert_up") "Insert field above")
   (structured-insert-up))
  ((balloon (icon "tm_insert_down") "Insert field below")
   (structured-insert-down))
  ((balloon (icon "tm_delete_up") "Remove field above")
   (prog-field-remove (focus-tree) #f))
  ((balloon (icon "tm_delete_down") "Remove field below")
...
(tm-menu (focus-extra-icons t)
  (:require (prog-field-context? t))
  (glue #f #f 8 0)
  (=> (balloon (icon "tm_input") "Input options")
      (link program-input-menu))
  (=> (balloon (icon "tm_output") "Output options")
      (link program-output-menu))
  (=> (balloon (icon "tm_session_session") "Program commands")
      (link program-program-menu))
  (glue #f #f 10 0)
  (=> (balloon (icon "tm_go") "Evaluate fields")
