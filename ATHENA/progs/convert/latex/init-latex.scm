...
(define (latex-athena-data-standard-enunciation-name var)
  (when (string? var) (set! var (string->symbol var)))
  (cond ((in? var '(theorem-name theoremname)) "Theorem")
        ((in? var '(proposition-name propositionname)) "Proposition")
        ((in? var '(lemma-name lemmaname)) "Lemma")
        ((in? var '(corollary-name corollaryname)) "Corollary")
        ((in? var '(axiom-name axiomname)) "Axiom")
        ((in? var '(definition-name definitionname)) "Definition")
        ((in? var '(notation-name notationname)) "Notation")
        ((in? var '(conjecture-name conjecturename)) "Conjecture")
        ((in? var '(law-name lawname)) "Law")
        ((in? var '(remark-name remarkname)) "Remark")
        ((in? var '(example-name examplename)) "Example")
        ((in? var '(note-name notename)) "Note")
        ((in? var '(warning-name warningname)) "Warning")
        ((in? var '(disambiguation-name disambiguationname))
         "Disambiguation")
        ((in? var '(convention-name conventionname)) "Convention")
        ((in? var '(acknowledgments-name acknowledgmentsname
                    acknowledgment-name acknowledgmentname))
         "Acknowledgments")
        ((in? var '(question-name questionname)) "Question")
        ((in? var '(answer-name answername)) "Answer")
        ((in? var '(exercise-name exercisename)) "Exercise")
        ((in? var '(problem-name problemname)) "Problem")
        ((in? var '(solution-name solutionname)) "Solution")
        ((in? var '(proof-name proofname)) "Proof")
        (else #f)))

(define (latex-athena-data-obsolete-enunciation-name? st)
  (and (func? st 'assign 2)
       (let ((expected
              (latex-athena-data-standard-enunciation-name (cadr st)))
             (definition (caddr st)))
         (and expected
              (func? definition 'macro 1)
              (== (cadr definition) expected)))))

(define (latex-athena-data-lyx-zero-width-space? st)
  (and (list? st)
       (null? (cdr st))
       (or (== (car st) 'LyXZeroWidthSpace)
           (== (car st) "LyXZeroWidthSpace"))))

(define (latex-athena-data-lyx-zero-width-space-definition? st)
  (and (func? st 'assign 2)
       (let ((name (cadr st))
             (definition (caddr st)))
         (and (or (== name 'LyXZeroWidthSpace)
                  (== name "LyXZeroWidthSpace"))
              (func? definition 'macro 1)
              (func? (cadr definition) 'space 1)
              (== (cadr (cadr definition)) "0pt")))))

(define (latex-athena-data-empty-structural-node? st)
  (or (== st "")
      (and (list? st)
           (in? (car st) '(concat para document hide-preamble))
           (list-and (map latex-athena-data-empty-structural-node?
                          (cdr st))))))

(define (latex-athena-data-normalize-legacy-tags texmacs-tree)
  (define (rewrite st)
    (cond ((or (latex-athena-data-obsolete-enunciation-name? st)
               (latex-athena-data-lyx-zero-width-space-definition? st)) "")
          ((latex-athena-data-lyx-zero-width-space? st) '(space "0pt"))
          ((and (list? st)
                (in? (car st) '(concat para document body hide-preamble)))
           (let* ((tag (latex-athena-data-normalize-legacy-tag (car st)))
                  (children
                   (list-filter (map rewrite (cdr st))
                                (lambda (x)
                                  (not (latex-athena-data-empty-structural-node?
                                        x))))))
             (if (and (== tag 'hide-preamble) (null? children)) ""
                 (cons tag children))))
          ((list? st)
