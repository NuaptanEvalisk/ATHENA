...
(define (tmtex-float-make wide? size type position x capt)
  (let* ((pos (string-replace position "f" ""))
         ...
         (figure-body* `(!paragraph (centering) ,body (caption ,caption)))
         (body* `(!paragraph ,body (caption ,caption))))
    (cond ((and (== size "big") (== type "figure"))
           (if (== pos "")
               `((!begin ,type) ,figure-body*)
               `((!begin ,type* (!option ,pos)) ,figure-body*)))
...
(define (mark-paired-middles l)
  (map (lambda (x)
         (if (func? x 'mid) (cons '!middle (cdr x)) x))
       l))
           ...
           (if (not n) (cons (car l) (pre-brackets (cdr l)))
               (let* ((r (pre-brackets (sublist l n (length l))))
                 ...
                 (if (disable-large? `(concat ,@m) 2)
                     ...
                     (append (mark-paired-middles m) r))))))
...
(define (tmtex-mid-sub l paired?)
  (if (tmtex-math-mode?)
      (let* ((s (tmtex-large-decode (car l)))
             (n (if (= (length l) 2) (string->number (cadr l)) 0))
             (b (cond ((and n (= n 1)) "bigm")
                      ((and n (= n 2)) "Bigm")
                      ((and n (= n 3)) "biggm")
                      ((and n (= n 4)) "Biggm")
                      (paired? "middle")
                      (else #f))))
        (if b (list (string->symbol (string-append b s)))
            (if (== s ".") "" s)))
      (tmtex-large-decode-text (car l))))
...
(define (tmtex-big-decode s)
  (cond ((nstring? s) "bignone")
        ((in? s '("sum" "prod" "int" "fint" "oint" "coprod")) s)
