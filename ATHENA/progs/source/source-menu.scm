...
(menu-bind source-icons
  (=> (balloon (icon "tm_assign") "Set or get environment variables")
      (link source-define-menu))
  (=> (balloon (icon "tm_lambda") "Write a macro")
      (link source-macro-menu))
  (=> (balloon (icon "tm_prime") "Control the evaluation of expressions")
      (link source-quote-menu))
  (=> (balloon (icon "tm_ctrl_flow") "Insert a control flow instruction")
      (link source-flow-menu))
  /
  (=> (balloon (icon "tm_three") "Insert a numerical operation")
      (link source-arithmetic-menu))
  (=> (balloon (icon "tm_textual") "Insert a textual operation")
      (link source-text-menu))
  (=> (balloon (icon "tm_tuple") "Insert an operation on tuples")
      (link source-tuple-menu))
  (=> (balloon (icon "tm_equal") "Insert a condition")
      (link source-condition-menu))
  /
  ((balloon (icon "tm_activate") "Activate")
   (make-mod-active 'active*))
  ((balloon (icon "tm_stretch") "Stretch")
   (make-style-with "src-compact" "none"))
  ((balloon (icon "tm_compact") "Compactify")
