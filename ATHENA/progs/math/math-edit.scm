          ...
          ((== (string-ref s (- i 1)) #\>)
           (let loop ((j (- i 1)))
             (cond ((<= j 0) 0)
                   ((== (string-ref s j) #\<) j)
                   (else (loop (- j 1))))))
          ((math-word-char? (string-ref s (- i 1)))
           (let loop ((j (- i 1)))
             (if (and (> j 0) (math-word-char? (string-ref s (- j 1))))
                 (loop (- j 1))
                 j)))
          (else (- i 1)))))

(define (math-select-token-before-cursor)
  (let* ((p (cursor-path))
         (i (and (nnull? p) (cAr p)))
         (q (and (nnull? p) (cDr p)))
         (t (and q (path->tree q))))
    (cond ((and (integer? i) (tree-atomic? t))
           (let* ((s (tree->string t))
                  (n (string-length s))
                  (end (max 0 (min i n)))
                  (start (math-token-start s end)))
             (and (> end start)
                  (begin
                    (selection-set (rcons q start) (rcons q end))
                    #t))))
          ((and (integer? i) (tree? t) (not (tree-atomic? t))
                (> i 0) (<= i (tree-arity t)))
           (let ((u (tree-ref t (- i 1))))
             (and u (tree-select u) #t)))
          (else #f))))

(tm-define (math-kbd-select-enlarge)
  (if (selection-active-any?)
      (select-enlarge)
      (or (math-select-token-before-cursor)
          (kbd-select-enlarge))))
