  ...
  (with j (string-previous s k)
    (if (== j 0) j
        (with i (string-previous s j)
          (cond ((and (string-alpha? (utf8-byte-substring s j k))
                      (or (string-alpha? (utf8-byte-substring s i j))
                          (== (utf8-byte-substring s i j) "<mathd>")))
                 (string-previous* s j))
                ((and (string-number? (utf8-byte-substring s j k))
                      (string-number? (utf8-byte-substring s i j)))
                 (string-previous* s j))
                (else j))))))
               ...
               (if (or (not (tree-atomic? u)) (tree-empty? u))
                   ...
                   (let* ((s (tree->string u))
                     ...
                     (if (and (tree-in? (path->tree (rcons q (+ j 1)))
                                        '(around around*))
                              (not (math-symbol? (utf8-byte-substring s k n)))
                              (not (math-operator? (utf8-byte-substring s k n))))
