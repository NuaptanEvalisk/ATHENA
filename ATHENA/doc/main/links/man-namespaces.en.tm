...
<\body>
  ...
  sitemap settings, an entrypoint, an optional post-generation command, and a
  regeneration mode. The selector is built visually from recursive path
  ...
  <ATHENA> writes a standards-compliant <verbatim|sitemap.xml> using absolute
  URLs below that base URL. If sitemap generation is disabled, no sitemap is
  written.
