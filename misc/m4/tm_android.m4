AC_DEFUN([TM_ANDROID], [
    ...
    if test "x$SVNINT" != "x"; then
      AC_SUBST([ATHENA_ANDROID_VERSION_CODE], [$SVNINT])
    elif test "x$ATHENA_ANDROID_VERSION_CODE" != "x"; then
      AC_SUBST([ATHENA_ANDROID_VERSION_CODE], [$ATHENA_ANDROID_VERSION_CODE])
    else
      AC_MSG_ERROR([Please set $ATHENA_ANDROID_VERSION_CODE shell variable.])
    fi
