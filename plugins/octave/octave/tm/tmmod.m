...
function info= tmmod (choice)
  ...
  global ATHENA_OCTAVE_PLUGIN_CONTROL;  # 28-August 2018: now using global variables - 1 Sept 2018: changed variable name
  if (nargin>0)   # change the plugin mode
    if (strcmpi (choice,'i') || choice==1) # ----------------------------- INTERACTIVE MODE -------------------------
      ATHENA_OCTAVE_PLUGIN_CONTROL= bitset (ATHENA_OCTAVE_PLUGIN_CONTROL,2,1); # set bit 2 to 1, figures are visible
      ATHENA_OCTAVE_PLUGIN_CONTROL= bitset (ATHENA_OCTAVE_PLUGIN_CONTROL,1,1); # set bit 1 (the lowest) to 1
    ...
    elseif (strcmpi (choice,'n') || choice==0) # ----------------------------- NORMAL MODE --------------------------
      ATHENA_OCTAVE_PLUGIN_CONTROL=bitset(ATHENA_OCTAVE_PLUGIN_CONTROL,1,0); # set bit 1 (the lowest) to 0
      if (any (strcmp("qt", graphics_toolkit())))  # Lorenzo added 1-9-2019, check if qt graphic toolkit is active
        ATHENA_OCTAVE_PLUGIN_CONTROL=bitset(ATHENA_OCTAVE_PLUGIN_CONTROL,2,0); # set bit 2 to 0: figures are hidden
      endif
      message= "-- TeXmacs OctaveX plugin: set normal plotting mode";
      message= char(message,"-- use tmmod('n'|'i') to change plotting mode");
    elseif (strcmpi (choice,'_v_')) # --------------- figures are visible --------------------------------------------
    	ATHENA_OCTAVE_PLUGIN_CONTROL= bitset(ATHENA_OCTAVE_PLUGIN_CONTROL,2,1); # set bit 2 to 1
    	message= "Octave figure visible";
    elseif (strcmpi (choice,'_h_')) # --------------- figures are hidden ---------------------------------------------
      ATHENA_OCTAVE_PLUGIN_CONTROL= bitset(ATHENA_OCTAVE_PLUGIN_CONTROL,2,0); # set bit 2 to 0
      message= "Octave figure hidden";
    else
      message= 'please use a valid option';
    endif
  else   # check the plotting mode
    if (bitget (ATHENA_OCTAVE_PLUGIN_CONTROL,1)==0)   # check bit 1 state (plotting mode)
      message= "current plotting mode is normal";
    endif
    if (bitget (ATHENA_OCTAVE_PLUGIN_CONTROL,1)==1)   # check bit 1 state (plotting mode)
