...
scm_i_init_guile (void *base)
{
  ...
  /* ATHENA's C++ object graph and its legacy fast allocator are single-threaded.
     Keep the notifier installed by scm_init_finalizers(), which queues
     finalization on the Scheme thread that triggered collection.  Upstream
     Guile replaces it here with a dedicated finalizer thread; that thread can
     concurrently destroy ATHENA smobs and corrupt the allocator free lists.  */
}
