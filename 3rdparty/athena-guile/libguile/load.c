...
#define FALLBACK_DIR \
    ...
    if ((e = getenv ("ATHENA_GUILE_CACHE_PATH")))
      snprintf (cachedir, sizeof(cachedir), "%s", e);
    else if ((e = getenv ("XDG_CACHE_HOME")))
...
/* Some of ATHENA's Guile 1.8-era files define macro support in modules that
   are completed later in the bootstrap.  Loading their source is sufficient
   for the current process, but compiling them immediately can still be too
   early.  Keep the retry set in the runtime so ATHENA can flush it once, after
   the complete bootstrap, and use bytecode on subsequent starts.  */
static SCM athena_deferred_auto_compilations;

static void
athena_defer_auto_compilation (SCM source)
{
  SCM cursor;
  SCM previous;
  SCM updated;

  for (cursor = athena_deferred_auto_compilations;
       scm_is_pair (cursor); cursor = scm_cdr (cursor))
    if (scm_is_true (scm_equal_p (scm_car (cursor), source)))
      return;
  previous = athena_deferred_auto_compilations;
  updated = scm_cons (source, previous);
  scm_gc_protect_object (updated);
  athena_deferred_auto_compilations = updated;
  scm_gc_unprotect_object (previous);
}

unsigned long
scm_athena_flush_deferred_auto_compilation (void)
{
  SCM deferred = athena_deferred_auto_compilations;
  SCM pending = deferred;
  unsigned long compiled = 0;

  scm_gc_protect_object (SCM_EOL);
  athena_deferred_auto_compilations = SCM_EOL;
  while (scm_is_pair (pending))
    {
      if (scm_is_true (scm_try_auto_compile (scm_car (pending))))
        compiled++;
      pending = scm_cdr (pending);
    }
  scm_gc_unprotect_object (deferred);
  return compiled;
}
...
canonical_suffix (SCM fname)
{
  ...
  /* ATHENA ships its application bytecode below ATHENA/lib.  Do not key that
     cache by the absolute build/install prefix: doing so would make the .go
     files non-relocatable and would force every user to compile the Scheme
     corpus again.  For sources below ATHENA_GUILE_SOURCE_ROOT, use the source
  ...
  if (scm_is_true (compiled_thunk))
  ...
  else
    {
      ...
      if (scm_is_true (freshly_compiled))
        ...
        {
          SCM value = scm_primitive_load (full_filename);

          /* ATHENA's Guile 1.8-era macro corpus has a few modules whose
             compile-time tables are established by top-level evaluation in
             the same source file.  The first compilation therefore cannot
             succeed, while compilation immediately after source loading can.
             Persist that second result for future starts without evaluating
             the generated thunk twice in this process. */
          if (scm_is_false (scm_try_auto_compile (full_filename)))
            athena_defer_auto_compilation (full_filename);
          return value;
        }
    }
}
