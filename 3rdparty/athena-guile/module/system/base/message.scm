       ...
       (let-syntax ((emit
         ...
         `((unsupported-warning ;; a "meta warning"
           ...
           (shadowed-toplevel
            ...
            ;; ATHENA deliberately uses sequential top-level replacement for
            ;; defaults, conditional definitions, and user customizations.
            ;; Reporting those replacements as lexical shadowing is incorrect
            ;; for this private runtime's language model.
           ...
           (arity-mismatch
            ...
            ,(lambda (port loc name certain?)
               ;; ATHENA's Guile 1.8-compatible top level is sequential and
               ;; permits procedures to be rebound after a caller is compiled.
               ;; A speculative arity inferred from such a binding is therefore
               ;; not actionable.  Keep definite lexical arity failures while
               ;; avoiding false diagnostics for compatibility probes and
               ;; dynamically generated GUI procedures.
               (when certain?
                 (emit port
