               ...
               (let* ((r (cons '("placeholder" placeholder) r))
                 ...
                 (letrec* ((record-definition!
                           ...
                           (athena-sequential-top-level?
                            (lambda ()
                              (let ((variable (module-variable (current-module) '%athena-sequential-top-level?)))
                                (and variable ((variable-ref variable) (current-module))))))
                                     ...
                                     (cond
                                       ((memv key '(define-form))
                                        (let* ((id (wrap value w mod))
                                          ...
                                          (list (if (or (eq? m 'c&e) (athena-sequential-top-level?))
                     ...
                     (lambda (ftype fval fform fe fw fs fmod)
                       (let ((key ftype))
                         (cond
                           ...
                           ((memv key '(define))
                            (let* ((tmp e) (tmp-1 ($sc-dispatch tmp '(_ ((any . any) . any) any . each-any))))
                              (if (and tmp-1
                                       (apply (lambda (name inner-args outer-args e1 e2)
                                                (and (id? name)
                                                     (valid-bound-ids? (lambda-var-list inner-args))
                                                     (valid-bound-ids? (lambda-var-list outer-args))))
                                              tmp-1))
                                  (apply (lambda (name inner-args outer-args e1 e2)
                                           (syntax-type
                                            ...
                                            for-car?))
                                         tmp-1)
                                  (let ((tmp-1 ($sc-dispatch tmp '(_ any any))))
               ...
               (let ((key type))
                 (cond
                   ...
                   ((memv key '(define-form))
                    (if (let ((variable
                               (module-variable
                                (current-module) '%athena-sequential-top-level?)))
                          (and variable
                               ((variable-ref variable) (current-module))))
                        (let* ((id (wrap value w mod)) (var (syntax-expression id)))
                          (build-global-definition s mod var (expand e r w mod)))
                        (syntax-violation
                         #f
            ...
            (expand-macro
             (lambda (p e r w s rib mod)
               (letrec* ((decorate-source (lambda (x) (source-wrap x '(()) s #f)))
                 ...
                 (let* ((t-680b775fb37a463-e25 transformer-environment)
                        (t-680b775fb37a463-e26 (lambda (k) (k e r w s rib mod))))
                   (with-fluid*
                    t-680b775fb37a463-e25
                    t-680b775fb37a463-e26
                                            ...
                                            (call-with-values
                                             (lambda ()
                                               (expand-lambda-case
                                                ...
                                                mod
                                                get-formals
                                                (map (lambda (tmp-680b775fb37a463-10a4
                                                              tmp-680b775fb37a463-10a3
                                                              tmp-680b775fb37a463-10a2)
                                                       (cons tmp-680b775fb37a463-10a2
                                                             (cons tmp-680b775fb37a463-10a3 tmp-680b775fb37a463-10a4)))
                                                     e2*
                                                     e1*
                                                     args*)))
                                             (lambda (meta* else*)
                                 ...
                                 (let ((tmp ($sc-dispatch tmp-1 '(#(vector #(each (any . any))) any . each-any))))
                                   (if tmp
                                       ...
                                       (expand-simple-lambda
                                        ...
                                        req
                                        rest
                                        (acons 'source (syntax->datum e) meta)
                                        body)))))))))
                    tmp)
         ...
         (if tmp
             (apply (lambda (args e1 e2)
                      (call-with-values
                       ...
                       (lambda (meta lcase) (build-case-lambda s (acons 'source (syntax->datum e) meta) lcase))))
                    tmp)
             (syntax-violation 'lambda "bad lambda*" e)))))
    (global-extend
     'core
       ...
       (letrec* ((build-it
                  (lambda (meta clauses)
                    (call-with-values
                     ...
                     (lambda (meta* lcase)
                       (build-case-lambda s (acons 'source (syntax->datum e) (append meta meta*)) lcase))))))
         (let* ((tmp-1 e) (tmp ($sc-dispatch tmp-1 '(_ . #(each (any any . each-any))))))
           (if tmp
               (apply (lambda (args e1 e2)
                        (build-it
             ...
             (if tmp-1
                 (apply (lambda (k keyword pattern template)
                          (expand-syntax-rules
                           ...
                           k
                           '()
                           (map (lambda (tmp-680b775fb37a463-11c4 tmp-680b775fb37a463-11c3 tmp-680b775fb37a463-11c2)
                                  (list (cons tmp-680b775fb37a463-11c2 tmp-680b775fb37a463-11c3)
                   ...
                   (if (if tmp-1
                       ...
                       (apply (lambda (k docstring keyword pattern template)
                                (expand-syntax-rules
                                 ...
                                 k
                                 (list docstring)
                                 (map (lambda (tmp-680b775fb37a463-11dd
                                               tmp-680b775fb37a463-11dc
                                               tmp-680b775fb37a463-11db)
                                        (list (cons tmp-680b775fb37a463-11db tmp-680b775fb37a463-11dc)
                         ...
                         (if (if tmp-1 (apply (lambda (dots k keyword pattern template) (identifier? dots)) tmp-1) #f)
                             (apply (lambda (dots k keyword pattern template)
                                      (expand-syntax-rules
                                       ...
                                       k
                                       '()
                                       (map (lambda (tmp-680b775fb37a463-11f6
                                                     tmp-680b775fb37a463-11f5
                                                     tmp-680b775fb37a463-11f4)
                                              (list (cons tmp-680b775fb37a463-11f4 tmp-680b775fb37a463-11f5)
                                                    tmp-680b775fb37a463-11f6))
                                            template
                                            pattern
                                            keyword)))
                                    tmp-1)
                                                       ...
                                                       (let ((tmp ($sc-dispatch
                                                         ...
                                                         (if tmp
                                                             (apply (lambda (p)
                                                                      (if (= lev 0)
                                                                          (quasilist*
                                                                           (map (lambda (tmp-680b775fb37a463-12c5)
                                                                                  (list "value"
                                                                                        tmp-680b775fb37a463-12c5))
                                                                                p)
                                                                           (quasi q lev))
                                                                          (quasicons
                                                                           (quasicons
                                                             ...
                                                             (let ((tmp ($sc-dispatch
                                                               ...
                                                               (if tmp
                                                                   (apply (lambda (p)
                                                                            (if (= lev 0)
                                                                                (quasiappend
                                                                                 (map (lambda (tmp-680b775fb37a463-12ca)
                                                                                        (list "value"
                                     ...
                                     (let ((tmp ($sc-dispatch
                                       ...
                                       (if tmp
                                           (apply (lambda (p)
                                                    (if (= lev 0)
                                                        (quasilist*
                                                         (map (lambda (tmp-680b775fb37a463-12e0)
                                                                (list "value" tmp-680b775fb37a463-12e0))
                                                              p)
                                                         (vquasi q lev))
                                                        (quasicons
                                                         (quasicons
                                           ...
                                           (let ((tmp ($sc-dispatch
                                             ...
                                             (if tmp
                                                 (apply (lambda (p)
                                                          (if (= lev 0)
                                                              (quasiappend
                                                               (map (lambda (tmp-680b775fb37a463-12e5)
                                                                      (list "value" tmp-680b775fb37a463-12e5))
                                                                    p)
                                                               (vquasi q lev))
                                                              (quasicons
                                                               (quasicons
                          ...
                          (let f ((y x)
                                  (k (lambda (ls)
                                       (let ((tmp-1 ls))
                                         (let ((tmp ($sc-dispatch tmp-1 'each-any)))
                                           (if tmp
                                               (apply (lambda (t-680b775fb37a463-132e)
                                                        (cons "vector" t-680b775fb37a463-132e))
                                                      tmp)
                                               (syntax-violation
                                                #f
                                                "source expression failed to match any pattern"
                            ...
                            (let ((tmp y))
                              (let ((tmp-1 ($sc-dispatch tmp '(#(atom "quote") each-any))))
                                (if tmp-1
                                    (apply (lambda (y)
                                             (k (map (lambda (tmp-680b775fb37a463-133a)
                                                       (list "quote" tmp-680b775fb37a463-133a))
                                                     y)))
                                           tmp-1)
                                                    ...
                                                    (let f ((x* x))
                                                      (if (null? x*)
                                                          ...
                                                          (let ((tmp-1 (list (emit (car x*)) (f (cdr x*)))))
                                                            (let ((tmp ($sc-dispatch tmp-1 '(any any))))
                                                              (if tmp
                                                                  (apply (lambda (t-680b775fb37a463-136c
                                                                                  t-680b775fb37a463-136b)
                                                                           (list (make-syntax
                                                                                  'cons
                                                                                  '((top))
                                                                                  '(hygiene guile))
                                                                                 t-680b775fb37a463-136c
                                                                                 t-680b775fb37a463-136b))
                                                                         tmp)
                                                                  (syntax-violation
                                                                   #f
