             ...
             (symbol-append
              ...
              (string->symbol
               ...
               (number->string
                ...
                16)))))
          (define (athena-sequential-top-level?)
            (let ((variable
            ...
            (call-with-values
              ...
              (lambda (ftype fval fform fe fw fs fmod)
                (case ftype
                  ...
                  ((define)
                   (syntax-case e ()
                     ((_ ((name . inner-args) . outer-args) e1 e2 ...)
                      (and (id? #'name)
                           (valid-bound-ids?
                            (lambda-var-list #'inner-args))
                           (valid-bound-ids?
                            (lambda-var-list #'outer-args)))
                      ;; Guile 1.8 accepted curried procedure definitions,
                      ;; including a variadic inner argument list.  ATHENA's
                      ;; Scheme corpus uses this form for higher-order helpers.
                      (syntax-type
                       #'(define name
                           (lambda inner-args
                             (lambda outer-args e1 e2 ...)))
                       r w s rib mod for-car?))
                     ((_ name val)
                      (id? #'name)
             ...
             ((_ (x ...) e1 e2 ...)
              (let ((when-list (parse-when-list e #'(x ...))))
                (if (memq 'eval when-list)
                    ...
                    (expand-void))))))
          ((define-form)
           ;; Guile 1.8 accepted a top-level definition under an expression
           ;; such as `(if feature? (define ...))'.  TeXmacs modules use this
           ;; for optional compatibility definitions.  Preserve that language
           ;; rule only for ATHENA's explicitly marked sequential modules;
           ;; ordinary Guile modules retain Guile 3's strict behavior.
           (if (let ((variable
                      (module-variable
                       (current-module) '%athena-sequential-top-level?)))
                 (and variable
                            ...
                            (let lp ((body #'(e1 e2 ...)) (meta '()))
                              (syntax-case body ()
                                ...
                                ((#((k . v) ...) e1 e2 ...) 
                                 (lp #'(e1 e2 ...)
                                     (append meta (syntax->datum #'((k . v) ...)))))
                                (_ (expand-simple-lambda
                                    e r w s mod req rest
                     ...
                     (syntax-case e ()
                       ((_ args e1 e2 ...)
                        (call-with-values
                          ...
                          (lambda (meta lcase)
                            (build-case-lambda
                             s (acons 'source (syntax->datum e) meta)
                             lcase))))
                       (_ (syntax-violation 'lambda "bad lambda*" e)))))
