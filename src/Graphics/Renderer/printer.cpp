    ...
    tex_fonts ("?"), tex_font_chars (array<int>(0)), metadata ("")
{
  ...
  load_string ("$ATHENA_PATH/misc/convert/tex.pro", tex_pro, true);
  load_string ("$ATHENA_PATH/misc/convert/special.pro", special_pro, true);
  load_string ("$ATHENA_PATH/misc/convert/color.pro", color_pro, true);
  load_string ("$ATHENA_PATH/misc/convert/texps.pro", texps_pro, true);
  
  prologue   << "%!PS-Adobe-2.0";
  if (suffix (ps_file_name) == "eps")
    prologue << " EPSF-2.0";
  prologue   << "\n"
             << "%%Creator: TeXmacs " ATHENA_VERSION "\n"
