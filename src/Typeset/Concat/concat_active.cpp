...
build_locus (edit_env env, tree t, list<string>& ids, string& col, string &ref,
  ...
  // Only registration and visited-state lookup need an editor repository.
  {
    ...
    for (i=0; i<last; i++) {
      ...
      if (is_compound (arg, "id", 1) && !is_nil (env->link_env)) {
...
concater_rep::typeset_toc_notify (tree t, path ip) {
  if (N(t) != 2) { typeset_error (t, ip); return; }
  string kind = tree_to_verbatim (env->exec (t[0]), false, "utf-8");
  string title= tree_to_verbatim (env->exec (t[1]), false, "utf-8");
...
#define error_image(t) { \
  typeset_dynamic (tree (_ERROR, "bad image", t), ip); \
  return; \
}
...
concater_rep::typeset_image (tree t, path ip) {
  ...
  if (is_atomic (image_tree)) {
    ...
    if (is_none (image)) image= "$ATHENA_PATH/misc/pixmaps/unknown.ps";
  }
