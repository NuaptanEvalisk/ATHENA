...
page_item_rep::page_item_rep (box b2, array<lazy> f2, int n2):
  type (PAGE_LINE_ITEM), b (b2), spc (0), penalty (0),
  block_bg (false), fl (f2), nr_cols (n2) {}
page_item_rep::page_item_rep (tree t2, int nr_cols2):
  type (PAGE_CONTROL_ITEM), b (empty_box (decorate ())), spc (0),
  penalty (HYPH_INVALID), block_bg (false), nr_cols (nr_cols2), t (t2) {}
page_item_rep::page_item_rep (int tp2, box b2, space spc2, int pen2,
  ...
  block_bg (false), fl (fl2), nr_cols (nr_cols2), t (t2) {}
...
page_item copy (page_item l) {
  page_item r= page_item (l->type, l->b, l->spc, l->penalty,
                          l->fl, l->nr_cols, l->t);
  r->block_bg= l->block_bg;
  return r;
}
