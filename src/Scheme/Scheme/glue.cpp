...
// Raw values are used only by the generic Scheme type predicates.
#define TMSCM_ASSERT_TMSCM(a,b,c)
static tmscm tmscm_to_tmscm (tmscm value) { return value; }
using procedure= object;
#define TMSCM_ASSERT_PROCEDURE(value,arg,routine) \
  TMSCM_ASSERT (scm_is_true (scm_procedure_p (value)), value, arg, routine)
static procedure tmscm_to_procedure (tmscm value) {
  return tmscm_to_object (value);
}
static tmscm procedure_to_tmscm (procedure value) {
  return object_to_tmscm (value);
}

#include <athena-glue/glue_basic.cpp>
#include <athena-glue/glue_editor.cpp>
#include <athena-glue/glue_server.cpp>

#include <athena-glue/glue_native.cpp>
