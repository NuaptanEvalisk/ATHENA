  ...
  while (i < raw.size()) {
    ...
    if (raw[i] == ' ' && i + 2 < raw.size() && raw[i + 1] == '^') {
      ...
      if (j > i + 2 && (j == raw.size() || (j < raw.size() && isspace(raw[j])))) {
        flush_text();
        append_concat(out, make_aofm_inline_anchor_placeholder(raw.substr(i + 2, j - (i + 2))));
        i = j;
        continue;
      }
    }
...
convert_list_item_body(const AstPtr& ast) {
  ...
  if (extract_trailing_inline_anchor_text(joined, content, anchor)) {
    tree out(CONCAT);
    append_concat(out, convert_inline_from_raw(content));
    append_concat(out, make_aofm_inline_anchor_placeholder(anchor));
    return simplify_concat(out);
  }
